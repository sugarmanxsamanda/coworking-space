...
const brandIndex = brands.findIndex((item) => item.id === brandId);
const previousBrand = brands[(brandIndex - 1 + brands.length) % brands.length];
const nextBrand = brands[(brandIndex + 1) % brands.length];
...
detailRoot.innerHTML = `
  <section class="detail-hero">
    <div class="detail-hero-main">
      <div class="back-link-row">
        <a class="back-link" href="../index.html">가이드 홈으로 돌아가기</a>
      ...
      ${
        brand.ctaPrimaryUrl || brand.ctaSecondaryUrl
          ? `
        <div class="detail-cta-row">
          ${
            brand.ctaPrimaryUrl
              ? `<a class="button primary" href="${brand.ctaPrimaryUrl}" target="_blank" rel="noreferrer">${brand.ctaPrimaryLabel || "문의하기"}</a>`
              : ""
          }
          ${
            brand.ctaSecondaryUrl
              ? `<a class="button secondary" href="${brand.ctaSecondaryUrl}" target="_blank" rel="noreferrer">${brand.ctaSecondaryLabel || "공식 사이트 보기"}</a>`
              : ""
          }
        </div>
      `
    ...
    <div class="detail-hero-side">
      <div class="detail-panel">
        ...
        <p class="fact-note">예비 점주가 의사결정하기 쉽게 같은 기준으로 정리한 해석입니다.</p>
      </div>
  ...
  ${
    brand.inquiryTitle || brand.inquiryBody
      ? `
    <section class="detail-inquiry-card">
      <div>
        <p class="section-kicker">Inquiry Focus</p>
        <h2>${brand.inquiryTitle || "상담 유도 섹션"}</h2>
        <p>${brand.inquiryBody || ""}</p>
      </div>
      <div class="detail-inquiry-actions">
        ${
          brand.ctaPrimaryUrl
            ? `<a class="button primary" href="${brand.ctaPrimaryUrl}" target="_blank" rel="noreferrer">${brand.ctaPrimaryLabel || "문의하기"}</a>`
            : ""
  ...
  <section class="detail-nav">
    <a class="nav-card" href="${previousBrand.href.replace("./", "../")}">
      <span class="nav-label">이전 브랜드</span>
      <strong>${previousBrand.name}</strong>
      <p>${previousBrand.positioning}</p>
    </a>
    <a class="nav-card" href="${nextBrand.href.replace("./", "../")}">
      <span class="nav-label">다음 브랜드</span>
      <strong>${nextBrand.name}</strong>
      <p>${nextBrand.positioning}</p>
    </a>
  </section>
`;
